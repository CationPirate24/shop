...
<div class="page-container">
	<form id="id3" action="user/tocheckList.do" method="post">
 		<input type="hidden" name="chaxunid" id="sId"/>
 		</form>
	<div class="text-c"> <span class="select-box inline">
		<select name="sect" class="select">
			<option value="0">全部分类</option>
			<option value="1">耳坠</option>
			<option value="2">项链</option>
			<option value="3">戒指</option>
			<option value="4">手表</option>
			<option value="5">头饰</option>
			<option value="6">手镯</option>
			<option value="7">手链</option>
		</select>
		</span> 日期范围：
		<input type="text" onfocus="WdatePicker({maxDate:'#F{$dp.$D(\'logmax\')||\'%y-%M-%d\'}'})" id="logmin" class="input-text Wdate" style="width:120px;">
		-
		<input type="text" onfocus="WdatePicker({minDate:'#F{$dp.$D(\'logmin\')}',maxDate:'%y-%M-%d'})" id="logmax" class="input-text Wdate" style="width:120px;">
		<input type="text" name="con" id="textId" placeholder=" 商品名称" style="width:250px" class="input-text">
		<button  id="searchId" class="btn btn-success" type="submit" onclick="search()"><i class="Hui-iconfont">&#xe665;</i> 搜商品</button>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		...
		<form id="de2" action="checkdelete.do" method="post"> 
 		<input type="hidden" name="deleteid" id="deId"/>
 		</form>
	 	
	 <a href="javascript:;" onclick="de()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a class="btn btn-primary radius"  data-title="修改商品" onclick="up()"  ><i class="Hui-iconfont">&#xe600;</i> 修改商品</a> <span class="r">共有数据：<strong>${size}</strong> 条</span>
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
				<c:if test="${empty muList}">
  				...
  				<c:if test="${!empty muList}">
  				<c:forEach items="${muList}" var="u">
				<tr class="text-c">
					<td><input type="checkbox"  name="check1" id="checkId" value="${u.id}"></td>
					<td>${u.name}</td>
					<td class="text-l">${u.textture}</u></td>
					<td>${u.type}</td>
					<td>${u.weight}</td>
					<td>${u.describe}</td>
					<td>${u.price}</td>
					...
					<td class="f-14 td-manage">
						<form id="myFormUpdate<%=count%>" action="toArticleUpdate.do" method="post">
  							...
  							<form id="myFormDelete<%=count%>" action="Articledelete.do" method="post">
  								<input type="hidden" name="id" value="${u.id}"/>
  							</form>
					<a style="text-decoration:none" onClick="article_stop(this,'${u.id}')"  title="下架"><i class="Hui-iconfont">&#xe6de;</i></a> <a style="text-decoration:none" class="ml-5" onClick="article_edit('<%=count%>')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" class="ml-5" onClick="article_del('<%=count++ %>')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
					</td>
				</tr>
				</c:forEach>
  			</c:if>
