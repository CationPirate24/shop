...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="de()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="member_add('添加用户','user/toMemberAdd.do','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加用户</a></span> <span class="r">共有数据：<strong>${count}</strong> 条</span> </div>
		<form id="de2" action="user/checkDeleteUser.do" method="post"> 
  ...
  <c:forEach items="${list}" var="m" varStatus="loop"> 
		...
		<tbody>
			<tr class="text-c">
				...
				<td class="td-manage">
					<form id="changePassword<%=count%>" action="user/toChangePassword.do">
					<input type="hidden" value="${m.name}" name="name1"/>
					<input type="hidden" value="${m.id}" name="id">
					</form>
					<form id="memberDelete<%=count%>" action="user/memberDelete.do" method="post">
				
					<input type="hidden" value="${m.id}" name="id"></form>
					<form action="user/memberUpdate.do"></form>
				<a style="text-decoration:none" onClick="member_stop(this,'10001')" href="javascript:;" title="停用">
					
				
				<i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" class="ml-5" onClick="change_password('修改密码','user/toChangePassword.do','10001','600','270','<%=count %>')" href="javascript:;" title="修改密码">
				
				<i class="Hui-iconfont">&#xe63f;</i></a> <a title="删除" href="javascript:;" onclick="member_del('<%=count++%>')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
...
/*密码-修改*/
function change_password(title,url,id,w,h,num){
	layer_show(title,url,w,h);	
	document.getElementById("changePassword"+num).submit();
}
/*用户-删除*/
